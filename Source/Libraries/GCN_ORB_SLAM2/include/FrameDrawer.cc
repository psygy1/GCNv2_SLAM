    ...
    if(state==Tracking::NOT_INITIALIZED) //INITIALIZING
    {
        ...
        for(unsigned int i=0; i<vMatches.size(); i++)
        {
            if(vMatches[i]>=0)
            {
                ...
                //cout << vCurrentKeys[vMatches[i]].pt << endl; //hack
                // save current points
                f << setprecision(7) << vCurrentKeys[vMatches[i]].pt << endl;
...
// save info
void FrameDrawer::SaveTextInfo(const string &filename){
    ofstream f(filename.c_str(), ios::app); 
    //f.open(filename.c_str());
    f << fixed;
    int nKFs = mpMap->KeyFramesInMap();
    int nMPs = mpMap->MapPointsInMap();
    int numMatches = mnTracked;
    if(mnTrackedVO>0)  numMatches += mnTrackedVO;
    if(numMatches < 0) numMatches = 0;
    //f << "KFs: " << nKFs << ", MPs: " << nMPs << ", Matches: " << mnTracked << endl;
    f << setprecision(1) << nKFs << " " << nMPs << " " << numMatches << endl;
    cout << nKFs << " " << nMPs << " " << numMatches << endl;
    f.close();
}
